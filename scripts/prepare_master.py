# -*- coding: utf-8 -*-
"""
【已废弃 — 2026-09-24 单库改造后不再使用】

废弃原因
    单库改造后，题库不再有「master 模板 + 每用户复制一份」这套结构。
    公共题库直接以 owner_id='__public__' 存在 data/exmsys.db 里，
    不存在从模板派生用户库的环节，也就没有「生成 master 模板」的需求。
    迁移完成后 data/master/ 已被归档到 data/archive/single_db_migration_*/。

保留原因
    万一将来需要从「某个完整库」重新提取一份纯题库（例如要把题库导出给
    另一个环境），里面的「清空个人记录表 + 删历史备份表」逻辑仍有参考价值。

⚠️ 请勿在单库模式下运行：它会去动 data/master/exmsys.db 并可能覆盖归档前的
   旧结构文件。真要复用请先另存副本、改掉路径常量。
