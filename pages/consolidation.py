...
def _show_consolidation_running():
    ...
    with main_col:
        ...
        # 题型标签 + 不确定按钮 + 标记按钮 同行
        type_col1, type_col2, type_col3 = st.columns([5, 2.5, 2.5])
        with type_col1:
    ...
    with card_col:
        render_answer_card(
            ...
            marked=st.session_state.consol_marked,
            uncertain=st.session_state.consol_uncertain,
            current_idx=idx,
...
def _show_consolidation_result():
    ...
    with st.container():
        ...
        st.markdown("---")
        # ---- 错题回顾（单列逐题浏览） ----
        review_idxs = [i for i, _q in enumerate(cq) if not results.get(_q["id"], {}).get("correct")]
        if review_idxs:
            st.markdown("---")
            ...
            for i, qi in enumerate(review_idxs):
                ...
                expander_title = f"{qi+1}. [{tp_label}] {q['question'][:60]}..."
                if tag:
                    if tag in ("消退型", "波动型"):
                        tag_emoji = "⚠️"
                    elif tag == "不确定题目":
