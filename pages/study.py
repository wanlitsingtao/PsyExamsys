...
from utils.data_manager import (
    ...
    load_config, load_study_records, save_study_record,
    update_study_record, find_study_record, infer_category,
    get_question_stats, get_all_question_stats,
)
...
def show_study():
    ...
    if "study_state" not in st.session_state:
        st.session_state.study_state = "idle"

    if st.session_state.study_state == "idle":
        _show_study_start(config)
        return

    if st.session_state.study_state == "running":
        _show_study_running(config)
        return

    if st.session_state.study_state == "finished":
        _show_study_result()
        return


def _show_study_start(config):
    """显示背题开始界面"""
    st.markdown("### 📖 准备开始背题练习")
    st.markdown("---")

    # 检查是否有未完成的背题
    resume_data = st.session_state.pop("_study_resume_data", None)
    if resume_data:
        st.info(f"📂 检测到上次未完成的背题会话（{resume_data.get('created_at','')[:16]}），是否继续？")
        col_r1, col_r2 = st.columns(2)
        if col_r1.button("✅ 继续上次练习", use_container_width=True, type="primary"):
            _resume_study(resume_data)
            return
    ...
    st.markdown("**练习规则：**")
    st.markdown(f"- 共 **{config['study_per_round']}** 题")
    st.markdown(f"  - 单选题 {config['study_single_count']} 题")
    st.markdown(f"  - 多选题 {config['study_multi_count']} 题")
    st.markdown(f"  - 判断题 {config['study_judge_count']} 题")
    st.markdown(f"  - 出题顺序：**单选题 → 多选题 → 判断题**")
    st.markdown(f"- 全部作答后统一提交")
    st.markdown(f"- 提交后可查看每题的正确答案和解析")
    st.markdown(f"- 中途退出自动保存进度")
    st.markdown("---")

    col1, col2, col3 = st.columns([1, 2, 1])
    if col2.button("🚀 开始练习", use_container_width=True, type="primary"):
        _start_new_study(config)


def _start_new_study(config):
    """开始新背题会话"""
    session_id = str(uuid.uuid4())[:8]
    total = config["study_per_round"]

    selected = extract_questions(
        st.session_state.questions,
        dan_count=config["study_single_count"],
        duo_count=config["study_multi_count"],
        pan_count=config["study_judge_count"],
        shuffle_types=False,
    )
    ...
    st.session_state.study_type_boundaries = {
    ...
    save_study_record({
        ...
        "answers": {},
        "config": {
            "study_per_round": total,
            "study_single_count": config["study_single_count"],
            "study_multi_count": config["study_multi_count"],
            "study_judge_count": config["study_judge_count"],
        },
    })
    st.rerun()


def _resume_study(record):
    """恢复上次未完成的背题会话"""
    config = st.session_state.config

    selected = extract_questions(
        st.session_state.questions,
        dan_count=record.get("config", {}).get("study_single_count", config["study_single_count"]),
        duo_count=record.get("config", {}).get("study_multi_count", config["study_multi_count"]),
        pan_count=record.get("config", {}).get("study_judge_count", config["study_judge_count"]),
        shuffle_types=False,
    )
...
def _show_study_running(config):
    """显示背题进行中的界面（与考试系统统一）"""
    sq = st.session_state.study_questions
    total_q = len(sq)
    idx = st.session_state.study_current
    q = sq[idx]
    qid = q["id"]
    boundaries = st.session_state.study_type_boundaries

    # 已答/未答统计
    answered = len(st.session_state.study_answers)
    ...
    if submit_check:
        st.session_state.study_confirm_submit = True

    if st.session_state.get("study_confirm_submit"):
    ...
    if q["type"] == "single" or q["type"] == "judge":
        ...
        selected_opt = st.radio(
            "请选择答案：",
            options_list,
            key=f"study_radio_{qid}",
            index=idx_in_list,
            label_visibility="collapsed",
        )
        if selected_opt:
            selected_key = selected_opt.split(":")[0]
            if st.session_state.study_answers.get(qid) != selected_key:
                st.session_state.study_answers[qid] = selected_key
                _sync_study_to_db()
        ...
        if selected_keys:
            ...
            if st.session_state.study_answers.get(qid) != ans_str:
                st.session_state.study_answers[qid] = ans_str
                _sync_study_to_db()
            st.caption(f"已选: {', '.join(selected_keys)}")
        else:
            st.caption("请选择至少一个选项")

    # 背题系统特有：查看答案功能（做题过程中随时可看）
    st.markdown("---")
    ...
    if st.session_state.get(show_answer_key):
        ...
        if st.button("🙈 收起答案", key=f"hide_ans_{qid}", use_container_width=False):
            st.session_state[show_answer_key] = False
            st.rerun()
    else:
        if st.button("📖 查看答案", key=f"show_ans_btn_{qid}", use_container_width=False):
    ...
    if nav_cols[0].button("◀ 上一题", use_container_width=True, disabled=(idx == 0)):
        st.session_state.study_current = idx - 1
        st.rerun()
...
def _finish_study():
    ...
    if session_id:
        update_study_record(session_id, {
            ...
            "results": {d["index"]: d for d in details},
        })
    ...
    st.session_state.study_state = "finished"
    st.rerun()


def _show_study_result():
    """显示背题结果（与考试系统统一）"""
    ...
    cols = st.columns(3)
    cols[0].metric("✅ 正确", f"{correct}/{total}", accuracy)
    ...
    for i, d in enumerate(details):
        ...
        correct_display = get_answer_display(
            d["type"], d["correct_answer"],
            # 从题库中找选项
            next((q["options"] for q in st.session_state.study_questions if q["index"] == d["index"]), {})
        )
        with st.expander(f"{i+1}. {status_icon} [{tp_label}] {d['question'][:60]}...", expanded=not d["is_correct"]):
            ...
            if options:
                ...
                q_data = next((q for q in st.session_state.study_questions if q["index"] == d["index"]), None)
                if q_data:
    ...
    if col2.button("🏠 返回首页", use_container_width=True, type="primary"):
        st.session_state.study_state = "idle"
        for key in ["study_questions", "study_answers", "study_result"]:
            if key in st.session_state:
                del st.session_state[key]
        st.rerun()


def _sync_study_to_db(status=None):
    """将当前背题状态同步到持久化JSON"""
    session_id = st.session_state.get("study_session_id", "")
    if not session_id:
        return
...
def _check_resume_study():
    """检查是否有未完成的背题会话，提供恢复选项"""
    if st.session_state.get("study_state") is not None and st.session_state.study_state != "idle":
        return  # 已经有活跃会话

    if st.session_state.get("_study_resume_checked"):
        return

    st.session_state._study_resume_checked = True
    ...
    latest = sorted(in_progress, key=lambda r: r.get("updated_at", ""), reverse=True)[0]
    st.session_state._study_resume_data = latest
